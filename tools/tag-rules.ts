...
import type { Parsed } from './parse-caption.ts';
  ...
  /* 出汁として使う素材は、その肉・魚を食べる料理にはしない。
     鶏ガラの粉末が入っているだけのパスタを「鶏肉」で出すと、押した人の期待から外れる。 */
  var STOCK_ONLY = /鶏ガラ|鶏がら|鶏出汁|鰹節|昆布|かつお節/;

  /* 材料名と料理名から引くタグ */
  var FROM_TEXT = [
    { tag: 'chicken',   re: /鶏|とり肉|チキン/ },
    { tag: 'pork',      re: /豚|ポーク/ },
    /* 牛乳を牛肉にしないため、牛のうしろに乳が来る場合だけ外す。
       カルビ・サーロイン・牛筋は「牛肉」と書かれないので、部位名でも引く。 */
    { tag: 'beef',      re: /牛(?!乳)|ビーフ|カルビ|ハラミ|サーロイン/ },
    { tag: 'seafood',   re: /海老|エビ|鮭|鰤|ぶり|ブリ|鯖|さば|サバ|鰆|鯛|いか|イカ|たこ|タコ|貝|あさり|ホタテ|まぐろ|マグロ|鮪|とろ|トロ|煮干し|魚/ },
    { tag: 'egg',       re: /^卵$|たまご|玉子/ },
    ...
    /* 「出汁」「煮汁」で終わる材料名を汁ものと数えない。
       出汁は和食のほぼ全部に入るので、素通しにすると汁もののタグが意味を失う。 */
    { tag: 'soup',      re: /味噌汁|スープ|お吸い物|(?<![出煮])汁$/ },
