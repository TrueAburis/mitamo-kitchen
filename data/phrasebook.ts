...
export const TEMPLATE_RULES: { title: string; body: string }[] = [
  {
    title: '先頭に、日本語と英語のタイトルを並べる',
    body: '1行目に【日本語のタイトル】、空行、2つ目に【English Title】を置いて、そのあと空行をあけて本文に入ります。Instagram はキャプションの1行目をアカウント名と同じ行に並べて表示するので、空行を入れないとタイトルが本文の書き出しにくっついて見えます。英語のタイトルを先頭に並べておくと、折り畳まれた状態でも海外の人が何の料理か分かります。英語の本文の前に置く [English Title] は、これまでどおり残します（英語だけを続けて読む人のため）。'
  },
  {
...
export const LOG: { date: string; text: string }[] = [
  {
    date: '2026-09-11',
    text: 'テンプレートを変更。先頭に日本語と英語のタイトルを並べる形にした（折り畳まれた状態でも海外の人に何の料理か分かるようにするため）。あわせてサイト側の取り込みを新しい形に対応させたので、【作り方】と [Steps] に書いていただいた手順が、そのままレシピページに入るようになった'
  },
  {
    date: '2026-09-07',
    text: 'この表がサイトの英語になるようにした。ここで直した英語が、そのままレシピページに出る（これまでは書き留めるだけで、画面は投稿のままだった）。あわせて20回分をレシピページに起こし、サイトのレシピは17件に。PRの3回は載せていない。煮干しラーメンの本文の対訳を追加し、広すぎた「使わない英語」を2つ狭めた（甘酢漬けの pickled と、出汁をかける the dashi stock が誤って引っかかっていたため）'
  },
  {
    date: '2026-09-07',
